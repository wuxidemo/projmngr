...
function submitcreatefinish(){
    ...
    xmlhttp.open("GET","submitcreatefinish?projectsn=" + projectsn + "&contractdate=" + contractdate + "&finishdate=" + finishdate + "&amount=" + amount);
...
function submitSearch(projectsn){
    ...
    xmlhttp.open("GET","getprojectinfo?projectsn="+$("#projectsn").val());
		...
		if(xmlhttp.status == 200){
			...
			if(json.length > 0){
				...
				if(msg == 'notexist'){
					...
					document.getElementById("contractdate").value=json[0].contractdate;
					document.getElementById("finishdate").value=json[0].finishdate;
...
<h2 class="form-signin-heading">请输入项目完工相关信息</h2>
<div class="container div">
    ...
    <div class="input-group date form_datetime">
         <span style="width:250px;text-align:right;" class="input-group-addon">合同日期:</span>
         <input type="text" class="form-control" style="width:350px;height:50px;" id="contractdate"  value="" readonly placeholder="请选择合同日期" />
		 <span class="input-group-addon" style="width:50px;"><span class="glyphicon glyphicon-th"></span></span>
    </div>
    <div class="input-group date form_datetime">
         <span style="width:250px;text-align:right;" class="input-group-addon">完工日期:</span>
         <input type="text" class="form-control" style="width:350px;height:50px;" id="finishdate"  value="" readonly placeholder="请选择完工日期" />
		 <span class="input-group-addon" style="width:50px;"><span class="glyphicon glyphicon-th"></span></span>
    </div>
            
    <div class="input-group input-group-lg">
         <span style="width:250px;text-align:right;" class="input-group-addon text-right">审定金额:</span>
         <input type="text" style="width:400px;text-align:left;" class="form-control" value="" placeholder="请输入审定金额" name="amount" id="amount" onkeyup="value=value.replace(/[^\d.]/g,'')"/>
    </div>    

    <br>
    <button style="width:620px;text-align:center;" class="btn btn-lg btn-primary btn-block" type="submit" onclick="submitcreatefinish()">项目完工录入</button><br>
