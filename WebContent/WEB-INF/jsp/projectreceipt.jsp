...
/*function submitcreatereceipt(){
	...
	var receiptbillamount = $("#receiptbillamount").val();
	var receiptamount = $("#receiptamount").val();
	var receipttype = $("#receipttype").val(); 
	if(receipttype== null || receipttype == "请选择收款方式"){
		receipttype = "";
	}
	...
	f.action = "submitcreatereceipt?projectsn=" + projectsn + "&projectvendor=" + projectvendor + "&receiptcontent=" + receiptcontent 
			   + "&receiptbillamount=" + receiptbillamount + "&receiptamount=" + receiptamount + "&receipttype=" + receipttype + "&description=" + description ;
	f.method = "POST";
	f.submit();
...
function submitcreatereceipt(){
    ...
    xmlhttp.open("POST","submitcreatereceipt?projectsn=" + projectsn + "&projectvendor=" + projectvendor + "&receiptcontent=" + receiptcontent 
			   + "&receiptbillamount=" + receiptbillamount + "&receiptamount=" + receiptamount + "&receipttype=" + receipttype + "&description=" + description);

	xmlhttp.onreadystatechange=getcreateresult;
	xmlhttp.send();
}
...
function getcreateresult(){
	if(xmlhttp.readyState == 4){
		if(xmlhttp.status == 200){
			...
			json = eval(json.createreceipt);
			if(json.length > 0){
				if(json[0].msg == 'billoverflow'){
					alert("开票金额超出范围，请检查！");
				}else if(json[0].msg == 'overflow'){
					alert("回款金额超出范围，请检查！");
				}else{
					alert("回款开票录入成功！");
					document.getElementById("receiptcontent").value = "";
					document.getElementById("receiptbillamount").value = "";
					document.getElementById("receiptamount").value = "";
					document.getElementById("receipttype").value = "请选择收款方式";
		...
		if(xmlhttp.status == 200){
			...
			if(json.length > 0){
				if(json[0].msg == 'notexist'){
					...
					document.getElementById("projectvendor").value = json[0].msg;	
				}
...
<h2 class="form-signin-heading">请输入回款开票相关信息</h2>
<div class="container div">
    ...
    <div class="input-group input-group-lg">
         <span style="width:250px;text-align:right;" class="input-group-addon">建设单位*:</span>
         <input type="text" style="width:400px;text-align:left;" class="form-control" value="" placeholder="请输入建设单位(必填)" name="projectvendor" id="projectvendor" />
    </div>        
    <div class="input-group input-group-lg">
         <span style="width:250px;text-align:right;" class="input-group-addon text-right">摘要:</span>
         <input type="text" style="width:400px;text-align:left;" class="form-control" value="" placeholder="请输入摘要" name="receiptcontent" id="receiptcontent" />
    </div>
    <div class="input-group input-group-lg">
         <span style="width:250px;text-align:right;" class="input-group-addon text-right">开票金额:</span>
         <input type="text" style="width:400px;text-align:left;" class="form-control" value="" placeholder="请输入开票金额" name="receiptbillamount" id="receiptbillamount" onkeyup="value=value.replace(/[^\d.]/g,'')"/>
    </div> 
    <div class="input-group input-group-lg">
         <span style="width:250px;text-align:right;" class="input-group-addon text-right">回款金额:</span>
         <input type="text" style="width:400px;text-align:left;" class="form-control" value="" placeholder="请输入回款金额" name="receiptamount" id="receiptamount" onkeyup="value=value.replace(/[^\d.]/g,'')"/>
    </div>    
    <div class="input-group input-group-lg">
         <span style="width:250px;text-align:right;" class="input-group-addon text-right">收款方式:</span>
         <select id="receipttype" style="width:400px;text-align:left;" class="selectpicker show-tick form-control" onchange="contracttypeselectOnChange()">
          <option selected disabled>请选择收款方式</option>
          <option>现金</option>
          <option>银行转帐</option>
          <option>承兑汇票</option>
          <option>发票</option>
    ...
    <br>
    <button style="width:620px;text-align:center;" class="btn btn-lg btn-primary btn-block" type="submit" onclick="submitcreatereceipt()">回款开票录入</button><br>
