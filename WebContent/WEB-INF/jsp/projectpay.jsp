...
function submitcreatepay(){
	...
	if(contracttype== null || contracttype == "请选择收款方式"){
		alert("请选择合同类型");
		return;
	}	
    ...
    xmlhttp.open("POST","submitcreatepay?projectsn=" + projectsn + "&payvendor=" + payvendor + "&paycontent=" + paycontent + "&contracttype=" + contracttype   
			   + "&paybillamount=" + paybillamount + "&payamount=" + payamount + "&paytype=" + paytype + "&description=" + description);
		...
		if(xmlhttp.status == 200){
			...
			if(json.length > 0){
				...
				if(msg != "" && msg != undefined){
					...
					document.getElementById("paycontent").value = "";
...
%>
<script>
    		alert("付款收票录入失败，请检查项目编号是否存在！");
</script>
<%     		
    }else if(result.equals("overflowbill")){
    	%>
    	<script>
    	    		alert("此项目供应商收票金额超出合同总额！");
    	</script>
    	<%     		
    }else if(result.equals("overflowpay")){
    	%>
    	<script>
    	    		alert("此项目供应商付款金额超出合同总额！");
    	</script>
    	<%     		
    }
    }
...
<h2 class="form-signin-heading">请输入付款收票相关信息</h2>
<div class="container div">
    ...
    <div class="input-group input-group-lg">
         <span style="width:250px;text-align:right;" class="input-group-addon">供应商*:</span>
         <input type="text" style="width:400px;text-align:left;" class="form-control" value="" placeholder="请输入供应商(必填)" name="payvendor" id="payvendor" />
    </div>        
    <div class="input-group input-group-lg">
         <span style="width:250px;text-align:right;" class="input-group-addon">合同类型*:</span>
         <select id="contracttypeselect" style="width:400px;text-align:left;" class="selectpicker show-tick form-control" >
    ...
    <div class="input-group input-group-lg">
         ...
         <input type="text" style="width:400px;text-align:left;" class="form-control" value="" placeholder="请输入收票摘要" name="paycontent" id="paycontent" />
    </div>
    <div class="input-group input-group-lg">
         <span style="width:250px;text-align:right;" class="input-group-addon text-right">收票金额:</span>
         <input type="text" style="width:400px;text-align:left;" class="form-control" value="" placeholder="请输入收票金额" name="paybillamount" id="paybillamount" onkeyup="value=value.replace(/[^\d.]/g,'')"/>
    </div> 
    <div class="input-group input-group-lg">
         <span style="width:250px;text-align:right;" class="input-group-addon text-right">付款金额:</span>
         <input type="text" style="width:400px;text-align:left;" class="form-control" value="" placeholder="请输入付款金额" name="payamount" id="payamount" onkeyup="value=value.replace(/[^\d.]/g,'')" />
    </div>    
    <div class="input-group input-group-lg">
         <span style="width:250px;text-align:right;" class="input-group-addon text-right">付款方式:</span>
         <select id="paytype" style="width:400px;text-align:left;" class="selectpicker show-tick form-control" >
          <option selected disabled>请选择付款方式</option>
          <option>现金</option>
          <option>银行转帐</option>
          <option>承兑汇票</option>
          <option>发票</option>
    ...
    <br>
    <button style="width:620px;text-align:center;" class="btn btn-lg btn-primary btn-block" type="submit" onclick="submitcreatepay()">付款收票录入</button><br>
